...
//    procedure dbg17DINUpdateData(Sender: TObject; var Text: string;
  ...
  private
    ...
    procedure WriteDJob(id_wc_owr: integer);
      //формирование суточного задания из предв плана
  public
    Vibor, id_newnom, Ins_Ord, id: Integer; //id для создания накладной 106
...
begin
  ...
  s := s + ' union ' +
    ...
    'cast(coalesce(o.name,''' + 'Без заказа' + ''') as varchar(64)) name_ord,' +
...
begin
  ...
  begin
    ...
    Open;
    if (not IsEmpty) and (MsgQuestion('План для этой операции уже существует.' +
      ' Запланировать повторно эту же операцию? ', 'Повторяющаяся запись') =
      id_no) then
    begin
    ...
    begin
      ...
      begin
        ...
        Add('select coalesce(coalesce(b.obj_name,i.name)||'' - ''||coalesce(b.dec_no,''''),'
          ...
          'r.fio,s.id_p,coalesce(r1.fio,''' + 'резерв''' + ') id_pFio,' +
          'sum(s.AMOUNT) amount' +
          ' from  p_area_bills p join  PA_BILL_POS s on p.id_pa_bill = s.id_pa_bill' +
...
begin
 ...
 //   Read(dbg15,iniGridEh);
  ...
  //Сортировка по полям при создании формы
...
begin
  ...
  fmDistrJob.Caption := 'Распределение работ на ' + DateToStr(dtp1.Date) +
    ' - ' + dmIS.qPANAME.AsString + '. (Персонально)';
  fmDistrJob.Kdistr := 1;
  fmDistrJob.Show;
end;
...
begin
  ...
  if not Assigned(fmDistrJob) then
    Application.CreateForm(TfmDistrJob, fmDistrJob);
  fmDistrJob.Caption := 'Распределение работ на ' + DateToStr(dtp1.Date) +
    ' - ' + dmIS.qPANAME.AsString + '. (По бригадам)';
  fmDistrJob.Kdistr := 2;
...
begin
  ...
  if dmIs.mT.InTransaction then
    dmIs.mT.CommitRetaining;
  if
    MsgQuestion('После подшивки суточного задания будет недоступна для редактирования. '
    +
      ...
      try
        ...
        begin
          ...
          if cbNakl.Checked then
            Open;
          if not IsEmpty then
            MsgInformation('Накладные на перемещение созданы.',
...
begin
  ...
  frxRepD_JOB.Script.Variables['D_JOB'] := 'на  ' +
...
begin
  ...
  begin
    ...
    ParamByName('NUMBER_').Value := qBillsNUMBER.AsString; //внутр. номер
...
begin
  ...
  frxRepDay_JOB.Script.Variables['D_JOB'] := 'на ' +
    FormatDateTime('dd  mmmm  yyyy', dtp1.Date) +
    '     ' + lcbShift.Text;
...
begin
  vibor := 0;
  fmD_JOB.Caption := 'Суточное задание: ' + fmMain.lcb1.Text;
  ...
  begin
    ShowMessage('Необходимо заполнить справочник смен...');
    exit;
  end
  else
...
begin
  ...
  begin
    ShowMessage('Вы не можете создавать накладные (создание накладной в пр-ме "Склад"). '
      +
      'Участок работает в реальном времени...');
    Exit;
  end;
  if not qBills.IsEmpty then
    ...
    begin
      ...
      if not IsEmpty then
        ...
        MsgInformation('Деталей на перемещение нет.',
          'Накладные на перемещение');
      if dmIS.mT.InTransaction then
        dmIS.mT.CommitRetaining;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage('Необходимо выделить записи...');
...
begin
  ...
  if (frxRepBills_s.Pages[1].Visible = True)
    {//  and (dmIs.qZaBillsPA_BILL_TYPE.AsInteger <> 110 )}then //накладная на склад (Кол-во)
  begin
    qIns.Close;
    qIns.SQL.Clear;
...
begin
  ...
  frxRep_ord_Det_TMC_.Script.Variables['D_JOB'] := 'на ' +
    FormatDateTime('dd  mmmm  yyyy', dtp1.Date) +
    '     ' + lcbShift.Text +
    '     ' + lcbEqp.Text;
  ...
  frxRepDay_JOB.Script.Variables['N_PROD_AREA'] := fmMain.lcb1.Text;
  frxRepDay_JOB.Script.Variables['D_JOB'] := 'на ' + FormatDateTime('dd  mmmm  yyyy', dtp1.Date)+
                                           '     ' + lcbShift.Text +
                                           '     ' + lcbEqp.Text;
...
begin
  if (dmIS.qD_JOB.Active = True) and (dmIS.dsD_JOB.State in [dsEdit, dsInsert])
    then
    dmIS.qD_JOB.Post; //запись данных в таблицу
  if (qPlan_P.Active = True) and (dsPlan_P.State in [dsEdit, dsInsert]) then
    qPlan_P.Post;     //запись данных в таблицу

  if (qProd_Plan_Stop.Active = True) and (dsProd_Plan_Stop.State in [dsEdit,
    dsInsert]) then
    qProd_Plan_Stop.Post;   //запись данных в таблицу
  if (qPlan_P_Stop.Active = True) and (dsPlan_P_Stop.State in [dsEdit, dsInsert])
    then
    qPlan_P_Stop.Post;    //запись данных в таблицу

  if dmIS.mT.InTransaction then    //true если имеется хотябы одна еще не завершенная транзакция
    dmIS.mT.CommitRetaining;  //запись в БД
  if pcAll.ActivePage = tsWork then
  begin
    qStore_Det.Close;   //закрывает БД
    qOper_Mats.Close;         //закрывает БД
    qBills.Close;         //закрывает БД
    qDoc.Close;        //закрывает БД
    dmIs.qD_JOB.DisableControls;  // отсоединяет DataSet от всех компонентов
    fmD_JOB.OpenD_JOB;
    dmIs.qD_JOB.EnableControls;
  end;
    ...
    begin
      ...
      begin
        ...
        //    qPlan_P.Locate('id_p',qLoad_PID_P.AsInteger,[]);
            ...
            MsgInformation('Ошибка записи распределения работ',
              'Ошибка записи');
            if dmIS.mT.InTransaction then
...
begin
  ...
  if (value > 24)
  and (Pos('Почасовой наемный рабочий',qPlan_PFIO.AsString ) = 0) then
  begin
    MsgError('Не может превышать 24 часа','Проверка данных');
    handled := true;
  end;
      ...
      if dsPlan_P_Stop.State in [dsEdit, dsInsert] then
         ...
         if dsPlan_P_Stop.State in [dsEdit, dsInsert] then
          ...
          if fmD_job.qPlan_P_Stop.FieldByName('tin').AsDateTime > fmD_job.qPlan_P_Stop.FieldByName('tto').AsDateTime
                ...
                then
                     begin
                       showmessage('Время начала, не может , быть раньше времени окончания');
                       exit;
                     end else
             ...
             if  fmD_job.qPlan_P_Stop.FieldByName('din').AsDateTime > fmD_job.qPlan_P_Stop.FieldByName('dto').AsDateTime
                then
                     begin
                       showmessage('Дата начала, не может , быть раньше даты окончания');
                       exit;
                     end else
           //koly86
...
begin
  ...
  begin
    MsgInformation('Заполните поле "Вид работ"', 'Проверка');
      ...
      begin
        ...
        begin
          ...
          try
            ...
            MsgInformation('Ошибка записи распределения внеплановых работ',
              'Ошибка записи');
            if dmIS.mT.InTransaction then
              dmIS.mT.RollbackRetaining;
          end; //try
...
begin
  ...
  begin
    ...
    Add('left join personnel r on r.id_p = p.id_p');
    //  Add('where s.id_prod_plan_stop = :id_prod_plan_stop'); долго работает
...
begin
  ...
  if (value > 24)
  and (Pos('Почасовой наемный рабочий',qPlan_P_StopFIO.AsString ) = 0) then
  begin
    MsgError('Не может превышать 24 часа','Проверка данных');
    handled := true;
  end;
...
begin
  ...
  lbItem.Caption := qBILLPOS_BILL70TRAFFN_ORDER.AsString + ' "' +
    qBILLPOS_BILL70TRAFFORDNAME.AsString + '" Деталь: ' +
    qBILLPOS_BILL70TRAFFOBJ_NAME.AsString + '/' +
    qBILLPOS_BILL70TRAFFONAME.AsString + ' Шаг ' +
    qBILLPOS_BILL70TRAFFID_STEP.AsString + ' Кол-во=' +
...
begin
  ...
  qIn.ParamByName('rem').AsString := 'На основании выработки от ' +
    DateToStr(dtp1.Date);
  ...
  mm.Text := 'На основании выработки' + DateToStr(dtp1.Date);
...
begin
  ...
  begin
    if
      MsgQuestion('После подшивки накладная будет недоступна для редактирования. '
      +
      'Все входящие в спецификацию объекты будут производить изменения состояния склада/участка. ' +
      SysVars.NReg +
    ...
    begin
      ...
      begin
        ...
        begin
          ShowMessage('Ошибка в количестве товара ' +
            FieldByName('ID_OBJECT').AsString +
            ' - ' + UpperCase(FieldByName('NAME').AsString) +
            ', партия № ' + FieldByName('ID_BATCH').AsString +
            ', на складе ' + FieldByName('SK').AsString +
            ', остаток = ' + FieldByName('OST').AsString);
          Screen.Cursor := crDefault;
          Exit;
        end;
        ...
        ParamByName('ras').AsInteger := 1; //расход
        try
          Open;
          if Transaction.InTransaction then
            Transaction.Commit;
          ...
          begin
            dmIs.qBill_ss.Locate('ID_OBJECT', FieldByName('status').AsInteger,
              []);
            Screen.Cursor := crDefault;
            MsgError('Ошибка при подшивке накладной. ТМЦ: (' +
              FieldByName('status').AsString + ') ' +
              dmIs.qBill_SsNAME.AsString, 'Ошибка подшивки.');
            if Transaction.InTransaction then
              Transaction.Rollback;
          end;
    ...
    begin
      Screen.Cursor := crDefault;
      ShowMessage('Накладную подшить нельзя...'); //D_FIX
    end;
...
begin
  ...
  if MsgQuestion(SysVars.NReg +
    ' , Вы действительно хотите удалить выбранную накладную?',
    'Удаление накладной') = id_yes then
  ...
  begin
    ...
    try
      ...
      remm := dmIs.qBillREM.AsString + ' Удалил: ' + SysVars.FReg + ', ' +
        DateTimeToStr(dtn);
      with qIns, qIns.SQL do
      begin
        Close;
      ...
      if dmIS.mT.InTransaction then
        dmIS.mT.RollbackRetaining;
      MsgError('Ошибка при удалении заявки.', 'Ошибка удаления.');
    end; //try
  end;
...
begin
  ...
  if (Text <> '') and (Value > dmIS.qD_JOBOST_ORD.Value) then
    ShowMessage('Остаток по не выполненным деталям составляет ' +
...
begin
  ...
  --o.obj_name = 'Труба 30х60х3 L=2028мм. ' o.id_object in (307568,307715)
  o.id_version=180189
...
if  fmd_job.dbg17.FieldColumns['P_TIME'].Color = clSkyBlue 
   ...
   then
     begin
        showmessage('Пожалуйста сохраните все данные');
        canclose:=false;
        exit;
